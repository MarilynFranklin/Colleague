...
class Checklist < Colleague
  ...
  def add_task(task)
    ...
    CSV.open('lib/tasks.csv', 'ab') do |csv|
      csv << [@@total_num_tasks, task.title, task.deadline.to_i, task.type, task.start_time.to_i, task.notes, task.status, task.project ? task.project.id : nil, task.time_estimate.to_i, task.dependent_task ? task.dependent_task.id : nil ]
    end
  end
